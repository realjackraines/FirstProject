...
class GmailThreadAnalyzer:
    ...
    def _generate_batch_summaries(self, messages):
        ...
        # Prepare batch prompt with individual summaries
        summaries = []
        for msg in messages:
            try:
                truncated_body = truncate_body(msg['body'])
                
                # Generate summary for individual message
                response = self.anthropic_client.messages.create(
                    model="claude-3-haiku-20240307",
                    max_tokens=300,
                    messages=[{
                        "role": "user", 
                        "content": f"""Provide a concise 1-2 sentence summary of this email.
                ...
                print(f"Error generating summary for message: {e}")
